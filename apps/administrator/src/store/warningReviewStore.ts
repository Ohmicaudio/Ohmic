import { create } from 'zustand'
import type { WarningReviewItem } from '@/types/intake'
import { fetchWarningReviewProjection } from '@/api/projections'

interface WarningReviewState {
  items: WarningReviewItem[]
  generatedAt: string | null
  loading: boolean
  error: string | null
  available: boolean
  fetch: () => Promise<void>
}

export const useWarningReviewStore = create<WarningReviewState>((set) => ({
  items: [],
  generatedAt: null,
  loading: false,
  error: null,
  available: false,

  fetch: async () => {
    set({ loading: true, error: null })
    try {
      const data = await fetchWarningReviewProjection()
      set({
        items: data.rows,
        generatedAt: data.generated_at,
        loading: false,
        error: null,
        available: true,
      })
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to load warning review'
      const isMissingProjection = message.includes('404')

      set({
        items: [],
        generatedAt: null,
        loading: false,
        error: isMissingProjection ? null : message,
        available: !isMissingProjection,
      })
    }
  },
}))
