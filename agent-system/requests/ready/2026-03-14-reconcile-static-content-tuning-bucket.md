scope: project
authority: working
project: ohmic-audio-static-content
status: ready
requested: 2026-03-14
requester: codex-local
origin: dependency
priority: soon
blocking: no
depends_on:
handoff_from:
claim_id: 20260314T032411Z-1ded7c5f
topic: requested-task

# Reconcile static-content `tuning` bucket

## Requested Outcome

- reconcile the `tuning/*` static pages between `ohmic-audio-labs` and `ohmic-audio-static-content`
- keep the stronger tuning workflow and reference copy in the static-host repo

## Scope

- `B:\ohmic\repos\ohmic-audio-labs\public\tuning`
- `B:\ohmic\repos\ohmic-audio-static-content\public\tuning`
- `B:\ohmic\docs\migration\STATIC_CONTENT_PARITY_AUDIT_2026-03-13.md`

## Notes

- differing files in this bucket: `26`
- this is one of the remaining unsplit top-level parity buckets after the first-wave closures
- examples include:
  - `tuning/index.html`
  - `tuning/beginner-level-frequency-response-basics/index.html`
  - `tuning/sections/4-4-calibration-and-verification/index.html`

## Ready When

- the canonical source is explicit for the bucket
- the reconciled `tuning/*` pages live in `ohmic-audio-static-content`
- follow-up diagrams or workflow-polish debt is captured separately if it is outside parity

## Suggested Claim Scope

- `B:\ohmic\repos\ohmic-audio-labs\public\tuning`
- `B:\ohmic\repos\ohmic-audio-static-content\public\tuning`
