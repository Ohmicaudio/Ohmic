...
- live BLE telemetry transport is validated on the Fire card
- this request is blocked by missing source data in firmware, not by BLE
  transport
- `setupAnalogADC()` is still stubbed while AmpLab envs were advertising
  `measurement_adc`
...
- exact blocker isolated:
  - no real measurement producer exists in the live headless AmpLab runtime
  - BLE transport is no longer the blocker
- follow-on packet is now
